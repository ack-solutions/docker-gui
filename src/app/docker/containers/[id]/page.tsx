...
const ContainerDetailContent = ({ containerId }: { containerId: string }) => {
  ...
  const handleRefresh = useCallback(async () => {
    await Promise.all([refetch(), inspectQuery.refetch()]);
  }, [refetch, inspectQuery]);
  ...
  return (
    <Stack spacing={3}>
      <BackButton label="Back to Containers" href="/docker/containers" />
      ...
      <Paper sx={{ px: 3, pt: 2, borderRadius: 3 }}>
        ...
        <TabPanel value="shell" selected={tab}>
          <Stack spacing={1.5}>
            ...
            <CommandTerminal
              sessionName={container?.name ?? inspectQuery.data.name}
              promptLabel={`root@${container?.name ?? inspectQuery.data.name}:/app`}
              welcomeMessage={`Welcome to the container shell. Connected to ${container?.name ?? inspectQuery.data.name}.`}
...
const ContainerDetailPage = () => {
  ...
  return <ContainerDetailContent containerId={containerId} />;
};
